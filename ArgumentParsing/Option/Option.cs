...
namespace ArgumentParsing.Option
{
    ...
    delegate bool ParseMethodDelegate<U>(string input, out U output);

    /// <summary>
    /// Generic class for parametrized options.
    /// </summary>
    internal class GenericStructParameterOption<T> : ParameterOption where T : struct
    {
        Action<T?> action;
        ...
        public GenericStructParameterOption(Action<T?> action, ParseMethodDelegate<T> parser)
        {
            this.action = action;
            this.parser = parser;
        }        
    ...
    internal class GenericClassParameterOption<T> : ParameterOption where T : class
    {
        ...
        public GenericClassParameterOption(Action<T?> action, ParseMethodDelegate<T> parser)
        {
            this.action = action;
            this.parser = parser;
        }
